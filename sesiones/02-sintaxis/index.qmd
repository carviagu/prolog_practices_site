...
Fuera de la terminal de PROLOG, ve a tu escritorio y en la carpeta llamada `prolog_labs` (que creamos en la sesión anterior), crea un nuevo archivo llamado `friends.pl` (Puedes crear un archivo de texto y renombrarlo cambiando la extensión).
...
Abre usando visual studio code el archivo `friends.pl` que creaste anteriormente.
...
Incorpora este hecho a tu archivo `friends.pl` como aparece aquí: 
...
En el archivo `friends.pl`. 
Ahora a ese hecho que acabas de añadir, incorpora dos nuevos hechos...

* Chandler ama a Mónica
* Joey ama a Rachel

Recuerda hacerso siguiendo la sintaxis que hemos definido. Cuando termines no olvides guardar. 
:::

###  Cargando el programa en PROLOG
Ahora que tenemos una primera versión del programa de PROLOG terminado podemos de cargarlo en la base de conocimiento de PROLOG para que podamos hacerle preguntas al respecto. 
...
Vamos a probar que nuestro programa funciona bien. Ahora que ya le hemos cargado a Prolog el conocimiento podemos preguntarle usando la misma sintaxis. 

La pregunta que nos hicimos era: *¿Ama Mónica a Chandler?*. Usando la sintaxis correspondiente preguntaselo a Prolog en la consola. ¿Qué devuelve ahora? Prueba a preguntarle si Rachel ama a Joey. ¿Qué responde? ¿Por qué?

:::

### Reglas (Rules)
Vamos a volver a nuestro programa en `friends.pl` Una vez tenemos nuestros hechos necesitamos dar a PROLOG unas reglas básicas para razonamiento. Las reglas nos permiten introducir razonamientos mediante los cuales Prolog a partir de los hechos puede inferir la respuesta. 
...
Incorpora esta regla a tu archivo `friends.pl` como aparece aquí. Debe de estar después de los hechos definidos: 
...
Ahora prueba definir una nueva regla: 

"Dos personas son amigos si almenos uno de ellos ama a el otro."

Recuerda hacerso siguiendo la sintaxis que hemos definido. Puedes ayudarte escribiéndola en Lógica de Primer Orden para después pasarlo a código como hemos hecho en el ejemplo. Cuando termines no olvides guardar. 
...
Después de guardar el archivo, cárgalo de nuevo en PROLOG de igual forma que hiciste antes. Verás que Prolog analiza los cambios y los aplica a su base de conocimiento.  

Ahora que ya tienes cargada la base de conocimiento friends, puedes preguntar a PROLOG usando la misma sintaxis y el responderá usando los hechos y reglas definidos. 
...
¿Qué te devuelve en cada caso? Recuerda que debes de usar la sintaxis que has diseñado para Prolog.
...
Prueba a preguntar a Prolog las siguientes preguntas y entiende como funcionan las trazas que explican el proceso de inferencia. 
...
Lo que acabamos de ver es la capacidad de Inferencia de Prolog. Prolog se basa en el conocimiento que le proporcionamos para "razonar" y tratar de responder a preguntas.

Cuando usamos el modo de trazas podemos ver que pasos sigue el motor de inferencia de Prolog para obtener la respuesta. 
...
* Una combinación de 3 platos es un menú, si está formada por un entrante, un segundo y un postre. 

Los hechos que conforman este programa son:

* La ensalada y la sopa son entrantes 
* El pollo es un plato de carne. 
* El rape y la lubina es un plato de pescado. 
...
* ¿Cuáles son los posibles menus que oferta el restaurante? ¿Cuántos hay?
