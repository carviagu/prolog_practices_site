...
Este documento asume que ya tienes instalado en tu ordenador los programas correspondientes, pero si necesitas instalar PROLOG y otros compnentes útiles para trabajar desde casa puedes ir a los @anexos de esta práctica.
:::

## Entorno de SWI PROLOG

Una vez instalado encontrarás distintas cosas en tu ordenador. Si usas el sistema operativo Windows puedes buscar SWI-Prolog y verás que te aparecerán los programas de la captura. Si eres usuario MacOS puedes encontrar SWI-Prolog en el Launchpad como una aplicación más. 
...
Fijate, que la instrucción sigue la sintaxis que ya hemos visto `<nombre_comando>.` . Prueba a escribir el comando `pwd` sin el punto. ¿Qué pasa? El cursor parece que sigue esperando que le des más información. Hasta que no termines con un `.` PROLOG no entenderá que has terminado de dar la instrucción y seguirá esperando a que entres más instrucciones en la terminal. 
...
-   Las rutas, para windows:  `cd('C:/Users/carviagu/Documents/pract_prolog').` o para mac: `cd(/Users/carviagu/Documents/pract_prolog).`,me llevarán a la carpeta `pract_prolog` localizada en Documentos, pues es lo que le indico dentro de la función.
...
Vamos a repasar las últimas tres funciones que hemos aprendido para ello:

1. Consulta en que directorio te encuentras usando el comando `pwd`

2. Intenta llegar al escritorio desde el directorio raiz usado el comando `cd().` *Nota:* Desde tu carpeta usuario (la que se abre por defecto cuando inicias PROLOG) verás que hay una carpeta llamada `Desktop` o `Escritorio` mediante la cual puedes llegar. 

3. Una vez te encuentres en el escritorio verificalo usando `pwd` y consulta los ficheros/directorios que hay disponibles usando `ls`. 

4. Crea una carpeta llamada `prolog_labs` usando el siguiente comando `make_directory('prolog_labs').` Verifica que la has creado correctamente usando `ls`

5. Entra en la carpeta que has creado y verifica que estás dentro usando los comandos correspondientes. 
...
Para introducir esta comparación en PROLOG, deberás de inicializar y comparar en el mismo comando:

```
?- A = 5, B = 5, A == B.
```

De esta forma PROLOG entenderá que primero debe de unificar las variables y después comparar sus valores. 
...
Dados los siguientes variables X con valor 9, Y con valor 4 e Z con valor 5.

1. Asigna a cada variable su valor correspondiente de tal forma que Prolog los memorize. *Nota:* Cuando hablamos de asignar nos referimos a unificar el valor en cada variable. 

2. Construye las comparaciones correspondientes para que Prolog responda true a las siguientes consultas:
    * Y es igual a 4 (comparativa)
    * Y es menor que X (comparativa)
    * X es igual al resultado de Y + Z (comparativa)

3. Si ahora introducimos una nueva variables J con valor `4 + 5`. Como sería la comparativa para que Prolog responda `true` al compararlo con Y + Z.

4. Es posible conseguir compara el la estructura J con la estructura Y + Z y que Prolog responda `true`. Justifica la respuesta. 
