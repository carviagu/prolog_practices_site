...
Escribe un programa que contenga la siguiente información:

1.  Utilizar el predicado **encargado_de_tarea(trabajador, tarea)**:
...
2.  Define las siguientes reglas que permitan comprobar ciertos hechos:

-   *Crea la regla con el predicado **encargado(Tarea)**, que nos permita verificar si una tarea ha sido encargada a alguien.*
-   *Crea la regla con el predicado **comparten_tarea(Persona1,Persona2)** que nos permita verificar si dos personas comparten alguna tarea.*

## Ejercicio 10
Dada la siguiente base de hechos: 

``` prolog
producto(laptop_hp, portatil).
producto(mac_air, portatil).
producto(pc_gaming, sobremesa).
producto(mac, sobremesa).
producto(teclado_logitech, periferico).
producto(raton_razer, periferico).
producto(monitor_samsung, monitor).
producto(monitor_hp, monitor).
```

Crea una regla que se llame **oferta(Producto1,Producto2)** que indique si hay una oferta comprando esos productos conjuntamente. Las actuales ofertas que hay son:

* Un ordenador de sobremesa y un periferico.
* Un ordenador de sobremesa y un monitor. 
* Un portatil y el periferico ratón razer. 


## Ejercicio 11

Crea estos programas que permitan obtener las operación aritmética indicadas: 

1. Mínimo de dos números.
2. Suma de dos números.
3. Verificar que el producto de dos números es igual a un tercer número. 

::: callout-tip
## Pista
Recuerda que necesitamos declarar tanto los valores de entrada como la salida en Prolog, es decir, en este caso si queremos Prolog nos devuelva el valor mínimo es necesario que definamos la variable objetivo en la llamada. 
